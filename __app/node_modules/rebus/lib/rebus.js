...
module.exports = function (folder, options, callback) {
  ...
  // Timeout ot wait for file recovery.
  ...
  */
  ...
  var closed = false;
  // Completion of loading when all files are loaded without errors.
  var _loadCompletion;
  // Files that didn't load successfully are referenced here.
  var errorWatcher = { errors: {} };
  // Store files that are in permanent error state.
  var permanentErrors = {};
  ...
  */
  ...
  function initializeAsync(instance, callback) {
    ...
    fs.readdir(folder, function (err, files) {
      ...
      _loadCompletion = function (err) {
        if (Object.keys(errorWatcher.errors).length === 0) {
          // No pending errors. This is the time to complete loading.
          _loadCompletion = function () { };
          if (Object.keys(permanentErrors).length !== 0) {
            err = new Error('Some files were not loaded');
          }
          callback(err);
        }
      }
  ...
  */
  ...
  function publish(prop, obj, callback) {
    ...
    fs.writeFile(fullname, JSON.stringify(obj, null, 2), function (err) {
      if (err) {
  ...
  */
  ...
  function _loadFile(filename, callback) {
    callback = callback || function () { };
    var permError = permanentErrors[filename];
    if (permError) {
      return callback(permError);
    }
    var filepath = path.join(folder, filename);
    function removeErrorState() {
      var errorState = errorWatcher.errors[filename];
      if (errorState) {
        // File that previously failed to load, now is loaded.
        clearTimeout(errorState.timeout);
        delete errorWatcher.errors[filename];
        // May need to complete asynchronous loading, if not completed yet.
        _loadCompletion();
      }
    }
    fs.readFile(filepath, function (err, data) {
      ...
      try {
        ...
        // Store this error to wait until file will be successfully loaded.
        var errorState = errorWatcher.errors[filename] || {};
        errorState.error = e;
        if (!errorState.timeout) {
          errorState.timeout = setTimeout(function () {
            // Call notifications with null to indicate that the object does not have value.
            _loadObject(filename, null);
            // No more attempts to load this file.
            permanentErrors[filename] = e;
            removeErrorState();
          }, options.timeout);
        }
        errorWatcher.errors[filename] = errorState;
  ...
  // if both obj and notification are null, there is a failure to load the object.
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (i === (length - 1)) {
        ...
        if (notification) {
          ...
          if (obj) {
            // Pin the object here.
            // If object failed to load, leave the last known object, still.
            refobj[prop] = obj;
          }
