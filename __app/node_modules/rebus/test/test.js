...
module.exports = testCase({
  ...
  sync1: function (test) {
    ...
    count++;
    var rebus1 = rebus(self.folder, { singletons: false }, function (err) {
      test.ok(!err, 'rebus should be instantiated normally');
      completion();
    });
  ...
  sync3: function (test) {
    ...
    count++;
    var rebusT = rebus(path.join(__dirname, 'testRebus'), function (err) {
      test.ok(!err, 'rebus should be instantiated normally');
      completion();
    });
  ...
  consistentNotification: function (test) {
    var self = this;
    var count = 1;
    function completion() {
      if (--count === 0) test.done();
    ...
    var rebuses = [];
    var notification1 = rebus1.subscribe('a', function (obj) {
      test.ok(obj, 'objet should be successfully published');
      var rebus2;
      try {
        count++;
        rebus2 = rebus(self.folder, { singletons: false }, function (err) {
          test.ok(!err, 'rebus should be instantiated normally');
          completion();
        });
        test.deepEqual(rebus2.value, { a: { b: { c1: 'x', c2: 'y'}} });
    ...
    count++;
    rebus1.publish('a.b', { c1: 'x', c2: 'y' }, function (err) {
      test.ok(!err, 'object should be published successfully');
      completion();
    });
  },
